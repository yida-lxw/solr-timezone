...
public class JSONResponseWriter extends org.apache.solr.response.JSONResponseWriter {
    ...
    protected FastWriter writer = null;

    public JSONResponseWriter() {
        this.contentType = CONTENT_TYPE_JSON_UTF8;
        this.writer = writer == null ? null: FastWriter.wrap(writer);
    }

    public JSONResponseWriter(String tz, String format) {
        this.tz = tz;
        this.format = format;
        this.writer = writer == null ? null: FastWriter.wrap(writer);
    }

    public JSONResponseWriter(String format) {
        this.format = format;
        this.writer = writer == null ? null: FastWriter.wrap(writer);
    }
    ...
    public void init(NamedList namedList) {
        ...
        String timezone = (String) namedList.get("tz");
        if (timezone != null && !"".equals(timezone)) {
            this.tz = timezone;
        }
        String format = (String) namedList.get("format");
    ...
    public void write(Writer writer, SolrQueryRequest req, SolrQueryResponse rsp) throws IOException {
        JSONWriter w = new JSONWriter(writer, req, rsp,this.tz,this.format);
        try {
            w.writeResponse();
        } finally {
            w.close();
...
class JSONWriter extends TextResponseWriter {
    ...
    public JSONWriter(Writer writer, SolrQueryRequest req, SolrQueryResponse rsp, String tz, String format) {
    ...
    public void writeDate(String name, Date val) throws IOException {
        String dateString = DateUtils.date2Timezone(val, TimeZone.getDefault().getID(),tz,format);
        writeDate(name,dateString);
    }
...
abstract class NaNFloatWriter extends JSONWriter {
    ...
    public NaNFloatWriter(Writer writer, SolrQueryRequest req, SolrQueryResponse rsp) {
        super(writer, req, rsp,null,null);
    }
